...
$(document).on("ready pjax:scriptcomplete", function () {
    ...
    try {
        ...
        console.error(
            "Could not parse language JSON - not on question editor page?"
        );
        return;
    }
    ...
    function updateRowProperties() {
        ...
        $(".answertable tbody").each(function loopTable() {
            ...
            $(this)
                .children("tr")
                .each(function loopTr() {
                    const uniqueRowId = $(this)
                        .data("common-id")
                        .split("_")
                        .shift();
                    ...
                    updateIfEmpty(
                        $(this).find(".oldcode"),
                        "name",
                        `oldcode_${uniqueRowId}_${scaleId}`
                    );
                    updateIfEmpty(
                        $(this).find(".code"),
                        "id",
                        `code_${uniqueRowId}_${scaleId}`
                    );
                    updateIfEmpty(
                        $(this).find(".code"),
                        "name",
                    ...
                    updateIfEmpty(
                        $(this).find(".answer"),
                        "id",
                        `answer_${language}_${uniqueRowId}_${scaleId}`
                    );
                    updateIfEmpty(
                        $(this).find(".answer"),
                        "name",
                    ...
                    updateIfEmpty(
                        $(this).find(".assessment"),
                        "id",
                        `assessment_${uniqueRowId}_${scaleId}`
                    );
                    updateIfEmpty(
                        $(this).find(".assessment"),
                        "name",
    ...
    function bindSubQuestionEvents() {
        $(".btnaddsubquestion")
            .off("click.subquestions")
            .on("click.subquestions", addSubquestionInput);
        $(".btndelsubquestion")
            .off("click.subquestions")
            .on("click.subquestions", deleteSubquestionInput);
    }

    function bindAnswerEvents() {
        $(".btnaddanswer")
            .off("click.subquestions")
            .on("click.subquestions", addAnswerOptionInput);
        $(".btndelanswer")
            .off("click.subquestions")
            .on("click.subquestions", deleteAnswerOptionInput);
    }
     ...
     */
    function fetchLabelSetPredefined(
        position /*: number */,
        source /*: string */
        ...
        if (source === "subquestions") {
            ...
            alert(
                "Internal error: source is not subquestions or answeroptions: " +
                    source
            );
            throw "abort";
        }
        ...
        if ($dataInput.length === 0) {
            alert(
                "Internal error: Found no data element in fetchLabelSetPredefined"
            );
            throw "abort";
        }
    ...
    function addinputQuickEdit(
        ...
        $.ajax({
            ...
            success(htmlrow) {
                const $langTable = $(
                    `#${tableIdPrefix}_${language}_${scaleId}`
                );
                $defer.resolve({
                    lang: language,
                    langtable: $langTable,
    ...
    function deleteSubquestionInput(e) {
        ...
        if (countanswers > 1) {
            ...
            _.forEach(languages, (curLanguage, x) => {
                const $tablerow = $(
                    `#row_${languages[x]}_${subquestionId}_${scaleId}`
                );
        ...
        if (countanswers > 1) {
            ...
            _.forEach(languages, (curLanguage, x) => {
                ...
                if ($tablerow.length === 0) {
                    ...
                    alert(
                        "Internal error: Could not find row to delete with id " +
                            rowId
                    );
                    throw "abort";
                }
     ...
     * @return {void}
     */
    function addNewInputAux(
        target /*: EventTarget */,
        data /*: {[string]: string} */,
        rebindClickHandler /*: () => void */
    ) {
    ...
    function isNumeric(mixedVar /*: mixed */) {
        return (
            (typeof mixedVar === "number" || typeof mixedVar === "string") &&
            mixedVar !== "" &&
    ...
    function getNextCode(sSourceCode) {
        ...
        const result =
            sourcecode.substr(0, sclength - foundnumberString.length) +
            foundnumberString;
        return result;
    }
    ...
    function showLabelSetPreview(lid /*: number */) /*: void */ {
        ...
        return $.ajax({
            ...
            cache: true,
            success(
                json /*: {success: bool, results: Array<{label_name: string, labels: Array<{code: string, title: string}>}>, languages: {}} */
            ) {
                if (json.success !== true) {
                    ...
                    const $aTemplate = $(
                        '<button type="button" role="tab" class="nav-link" data-bs-toggle="tab" data-bs-target=""></button>'
                    );
                    const $tabTodyTemplate = $("<div></div>");
                    const $listTemplate = $(
                        '<div class="list-group selector_label-list container-fluid"></div>'
                    );
                    const $listItemTemplate = $(
                        '<div class="row mb-3 selector_label-list-row"></div>'
                    );
                    const $tabindex = $(
                        '<ul class="nav nav-tabs" role="tablist"></ul>'
                    );
                    const $tabbody = $(
                        '<div class="tab-content" id="label-set-tab-content" style="max-height: 50vh; overflow:auto;"></div>'
                    );
                    ...
                    $.each(json.languages, (language, languageName) => {
                        ...
                        if (labelSet.labels) {
                            ...
                            labelSet.labels.forEach((label) => {
                                ...
                                $listItem.append(
                                    `<div class="col-lg-3 text-end" style="border-right: 4px solid #cdcdcd">${label.code}</div>`
                                );
                                // @ts-ignore
                    ...
                    if (isEmpty) {
                        ...
                        if (hasInvalidCodes) {
                            // @ts-ignore
                            showLabelSetAlert(
                                languageJson.answeroptions.truncationWarning
                            );
                        }
                        // @ts-ignore
                        $("<div></div>")
    ...
    function showLabelSetAlert(
        ...
        window.LS.ajaxAlerts(message, alertType, {
            inline: "#labelsetalert",
            class: "ls-space margin bottom-0 top-15",
        });
        alert.show();
    }
    ...
    function initLabelSetModal(e) {
        ...
        if ($("#current_scale_id").length === 0) {
            $("body").append(
                `<input type="hidden" id="current_scale_id" value="${scaleId}" name="current_scale_id" />`
            );
        } else {
            ...
            success(jsonString) {
                if (jsonString.success !== true) {
                    ...
                    jsonString.labelsets.forEach((item) => {
                        const newOption = $(
                            `<option value="${item.lid}">${item.label_name}</option>`
                        ); // jshint ignore: line
                        // @ts-ignore
    ...
    function transferLabels(
         ...
         */
        fetchLabelSetPredefined(1, source)
            .then((result) => {
        ...
        */
                    ...
                    if ($table.length === 0) {
                        alert(
                            "Internal error: Found no table to add labels to with id " +
                                tableId
                        );
                        throw "abort";
                    }
                    ...
                    let importedLabelset = $("#labelsetpreview")
                        .find(`#language_${lang}`)
                        .find(".selector_label-list")
                        .find(".selector_label-list-row");
                    if (importedLabelset.length === 0) {
                        importedLabelset = $("#labelsetpreview")
                            .find(`.tab-pane:first`)
                            .find(".selector_label-list")
                            .find(".selector_label-list-row");
                    }
                    importedLabelset.each((i, item) => {
                        try {
                            ...
                            if (source === "subquestions") {
                                ...
                                throw (
                                    "source is not subquestions or answeroptions: " +
                                    source
                                );
                            }
                            if ($tr.length === 0) {
                            ...
                            $tr.attr(
                                ...
                                $tr
                                    .attr("data-common-id")
                                    .replace(/-QUIDPLACEHOLDER-/, langIds[i])
                            );
                            // @ts-ignore
                            $tr.attr(
                                "id",
                            ...
                            if (
                                $tr
                                    .find("td.code-title")
                                    .find("input[type=text]").length > 0
                            ) {
                                $tr.find("td.code-title")
                            ...
                            if (
                                $tr
                                    .find("td.relevance-equation")
                                    .find("input[type=text]").length > 0
                            ) {
                                $tr.find("td.relevance-equation")
                                    .find("input[type=text]")
                                    .val(1);
                            } else {
                                // ??
                            ...
                            $tr.find(
                                "td.subquestion-text, td.answeroption-text"
                            )
                                .find("input[type=text]")
                                .val(label.title);
                            ...
                            if (source === "subquestions") {
                                ...
                                $table
                                    .find(".btndelsubquestion")
                                    .off("click.subquestions")
                                    .on(
                                        "click.subquestions",
                                        deleteSubquestionInput
                                    );
                            } else {
                                $table
                                    .find(".btnaddanswer")
    ...
    */
        ...
        if ($closestTable.find(".code").length < 0) {
            ...
            $closestTable.find(".code").each(function () {
                codes.push($(this).val());
                currentIds.push(
                    $(this).closest("tr").data("common-id").split("_").shift()
                );
            });
        }
        ...
        const codeSigil =
            codes[0] !== undefined ? codes[0].split("") : "001".split("");
        ...
        lsrows.forEach(
            (
                value /*: string & {splitCSV: string => Array<string>} */,
                k /*: number */
            ) => {
                ...
                if (thisrow.length <= languages.length) {
                    ...
                    let prefix = codeSigil
                        .slice(0, Math.max(0, 5 - qCode.length))
                        .join("");
                ...
                languages.forEach((language, x) => {
                    ...
                    if (lsreplace) {
                        if (tableId.indexOf("subquestion") > -1) {
                            $(
                                `#subquestions_${language}_${scaleId} tbody`
                            ).empty();
                        } else {
                            $(
                                `#answeroptions_${language}_${scaleId} tbody`
                            ).empty();
                        }
                    }
                    answers[language].push({
                        text: thisrow[parseInt(x) + 1],
                        code: thisrow[0],
                        quid,
                    });
                });
        ...
        $.when.apply($, promises).done(
            function (...args) {
                ...
                args.forEach(
                    (
                        item /*: {lang: string, langtable: {}, html: string} */
                    ) => {
                        answers[item.lang].forEach(
                            (
                                row /*: {quid: string, text: string, code: string} */
                            ) => {
                                try {
                                    const { html } = item;
                                    const htmlQuid = html.replace(
                                        /-QUIDPLACEHOLDER-/g,
                                        row.quid
                                    );
                                    ...
                                    if (!tableRow) {
                                        throw (
                                            "Could not find tableRow with id " +
                                            htmlQuid
                                        );
                                    }
                                    ...
                                    const inputText =
                                        tableRow.querySelector("input.answer");
                                    if (
                                    ...
                                    let inputCode =
                                        tableRow.querySelector("input.code");
                                    ...
                                    const relevanceEquation =
                                        tableRow.querySelector(
                                            "td.relevance-equation input"
                                        );
                                    if (
                                        relevanceEquation instanceof
                                        HTMLInputElement
                                    ) {
                                    ...
                                    alert(
                                        "Internal error in quickAddLabels:" + e
                                    );
                                    throw "abort";
                                }
                            }
    ...
    function saveAsLabelSetOptionClick(event /*: Event */) /*: void */ {
        ...
        switch (target.getAttribute("id")) {
            ...
            default:
                alert(
                    "Internal error: Unsupported id in target (saveAsLabelSetOptionClick)"
                );
                throw "abort";
        }
    }
    ...
    */
        ...
        $.ajax({
            ...
            success(successMessage) {
                LS.LsGlobalNotifier.createAlert(successMessage, "success", {
   ...
   */
    ...
    function aftermove(event, ui) {
        ...
        $that
            .closest(".tab-pane")
            .find(".extra-lang  table[data-scaleid='" + scaleId + "']")
            .each((idx, table) => {
                if (newindex < oldindex) {
                    // @ts-ignore
                    $(table)
                        .find(`tbody tr:nth-child(${newindex})`)
                        .before(
                            $(table).find(`tbody tr:nth-child(${oldindex})`)
                        );
                } else {
                    // @ts-ignore
                    $(table)
    ...
    function onClickSaveLabelSet(
        ...
        if (lid === 0) {
            ...
            $.post(
                languageJson.sCheckLabelURL,
                { languages: aLanguages, lid, bCheckAssessments: 1 },
                (data) => {
                    $("#strReplaceMessage").html(data);
                    // @ts-ignore
                    const modal = new bootstrap.Modal(
                        document.getElementById("dialog-confirm-replaceModal"),
                        {}
                    );
                    modal.show();
     ...
     */
    // @ts-ignore
    function checkSubquestionCodeUnique(
        table /*: HTMLElement */,
        msg /*: string */
    ) /*: boolean */ {
     ...
     */
    function createCheckUniqueFunction(
        msg /*: string */
    ) /*: (HTMLElement) => boolean */ {
        return (that) => {
            ...
            if (!checkSubquestionCodeUnique(table, msg)) {
                LS.LsGlobalNotifier.createAlert(msg, "danger", {
                    showCloseButton: true,
                });
                hasError = true;
            }
    ...
    LS.questionEditor = {
        ...
        updateQuestionAttributes: async function (
            questionType,
            questionTheme,
            generalSettingsUrl,
            advancedSettingsUrl,
            ...
            try {
                ...
                const [
                    ...
                    advancedSettingsPromise,
                    extraOptionsPromise,
                ]);
                const currentGroup = $("#gid")
                ...
                // @ts-ignore
                var runner_questionTypeSelector = new PreviewModalScript(
                    "questionTypeSelector",
                    {
                        ...
                        onUpdate: [
                            "value",
                            "theme",
                            "$('#question_type').val(value); $('#question_theme_name').val(theme); LS.questionEditor.updateQuestionAttributes(value, theme, '" +
                                generalSettingsUrl +
                                "', '" +
                                advancedSettingsUrl +
                                "', '" +
                ...
                $("#advanced-options-container").replaceWith(
                    advancedSettingsHtml
                );
                ...
                const languages = languageJson.langs.split(";");
                $(
                    '.lang-switch-button[data-lang="' + languages[0] + '"]'
                ).trigger("click");
        ...
        checkQuestionValidateTitle: function (code, qid) {
            ...
            $.ajax({
                ...
                error: (data) => {
                    alert(
                        "Internal error in checkQuestionValidateTitle: " +
                            JSON.stringify(data)
                    );
                    throw "abort";
                },
         ...
         * @return {boolean}
         */
        checkIfSaveIsValid: function (
            event /*: Event */,
            tabQuestionEditor = "editor"
        ) {
            $("#ls-loading").show();
            event.preventDefault();
            // @ts-ignore
            ...
            if (!(target instanceof HTMLElement)) {
                alert(
                    "Internal error in checkIfSaveIsValid: target is not an HTMLElement, but " +
                        typeof target
                );
                $("#ls-loading").hide();
                return false;
            ...
            const firstSubquestionRow = document.querySelector(
                ".subquestions-table tr"
            );
            if (firstSubquestionRow) {
                // This will show error message if subquestion code is not unique.
                if (
                    !LS.questionEditor.showSubquestionCodeUniqueError(
                        firstSubquestionRow
                    )
                ) {
                    $("#ls-loading").hide();
            ...
            const firstAnsweroptionRow = document.querySelector(
                ".answeroptions-table tr"
            );
            if (firstAnsweroptionRow) {
                // This will show error message if answer option code is not unique.
                if (
                    !LS.questionEditor.showAnswerOptionCodeUniqueError(
                        firstAnsweroptionRow
                    )
                ) {
                    $("#ls-loading").hide();
                    return false;
            ...
            const updateQuestionSummary = () => {
                ...
                $.ajax({
                    ...
                    error: (response) => {
                        $("#ls-loading").hide();
                        alert(
                            "Internal error in updateQuestionSummary: " +
                                response
                        );
                        return false;
                    },
            ...
            const saveFormWithAjax /*: (void) => (void) */ = () => {
                ...
                $("#edit-question-form")
                    .serializeArray()
                    .forEach((x /*: {name: string, value: string} */) => {
                        data[x.name] = x.value;
                    });
                ...
                $.post({
                    ...
                    success: (
                        ...
                        // Update the side-bar.
                        LS.EventBus.$emit("updateSideBar", {
         ...
         */
        showSubquestionCodeUniqueError: createCheckUniqueFunction(
            languageJson.subquestions.duplicatesubquestioncode
        ),
        showAnswerOptionCodeUniqueError: createCheckUniqueFunction(
            languageJson.answeroptions.duplicateanswercode
        ),
    };
    ...
    $("#quickaddModal").on("show.bs.modal", (e) => {
        ...
        // @ts-ignore
        const tableId = $(e.relatedTarget)
            .closest("div.action-buttons")
            .parent()
            .find("table.answertable")
            .attr("id");
        if (tableId === "") {
    ...
    $(".lang-switch-button").on("click", function langSwitchOnClick(e) {
        ...
        $(this)
            .closest("ul")
            .find("li")
            .each(function removeActiveClassFromLanguageListItems() {
    ...
    $(document).on(
        "focusout",
        "#subquestions table.subquestions-table:first-of-type td.code-title input.code",
        syncAnswerSubquestionCode
    );
    $(document).on(
        "focusout",
        "#answeroptions table.answeroptions-table:first-of-type td.code-title input.code",
        syncAnswerSubquestionCode
    );
});
...
//         $(document).off('.unsavedChanges');
//         $(window).off('.unsavedChanges');
//         $('a[data-bs-toggle="tab"]').off('.unsavedChanges');
//     }

//     cleanupHandlers();

//     // Single handler for form changes
//     $(document).on('change.unsavedChanges input.unsavedChanges',
//         '#edit-question-form :input:not([type=submit])',
//         function() {
//             hasUnsavedChanges = true;
//     });

//     // Single handler for CKEDITOR changes
...
//     // Reset flag when saved
//     $('#edit-question-form').on('submit.unsavedChanges', function() {
//         hasUnsavedChanges = false;
//     });

//     // Reset flag after successful Ajax save
//     $(document).on('ajaxComplete.unsavedChanges', function(event, xhr, settings) {
//         if (settings.url?.indexOf('saveQuestionData') > -1 && xhr.responseJSON?.success) {
//             hasUnsavedChanges = false;
//         }
//     });

//     // Single handler for all navigation attempts
//     async function handleNavigation(e) {
//         if (hasUnsavedChanges && !isPromptActive) {
//             e.preventDefault();
//             e.stopPropagation();
...
//     // Single handler for tab switching
//     $(document).on('show.bs.tab.unsavedChanges', 'a[data-bs-toggle="tab"]', async function(e) {
//         if (!await handleNavigation(e)) {
//             e.preventDefault();
...
//     $(document).on('click.unsavedChanges', 'a:not(.ignore-changes)', async function(e) {
...
//     // Single handler for page unload
...
$(document).one("ready pjax:scriptcomplete", function () {
    const unsavedChangesMonitor = (function () {
        let hasUnsavedChanges = false;
        let isPromptActive = false;
        ...
        // Single handler for form changes
        $(document).on(
            "change.unsavedChanges input.unsavedChanges",
            "#edit-question-form :input:not([type=submit])",
            function () {
                hasUnsavedChanges = true;
            }
        ...
        // Reset flag after successful Ajax save
        $(document).on(
            "ajaxComplete.unsavedChanges",
            function (event, xhr, settings) {
                if (
                    settings.url?.indexOf("saveQuestionData") > -1 &&
        ...
        // Single handler for tab switching
        $(document).on(
            "show.bs.tab.unsavedChanges",
            'a[data-bs-toggle="tab"]',
            async function (e) {
                const result = await handleNavigation(e);
                if (!result) {
                    e.preventDefault();
                    e.stopImmediatePropagation();
                    return false;
        ...
        // Single handler for clicks
        $(document).on(
            "click.unsavedChanges",
            "a:not(.ignore-changes)",
            async function (e) {
                const href = $(this).attr("href");
                if (href && href !== "#" && !href.startsWith("javascript:")) {
                    const result = await handleNavigation(e);
    ...
    window.unsavedChangesMonitor = unsavedChangesMonitor;
});
